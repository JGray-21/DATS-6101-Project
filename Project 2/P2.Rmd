...
pred <- predict(gssfit, type = "class")
mat <- confusionMatrix(gss$pred)
mat
```
...
gssfit <- rpart(confed ~ age + income + educ, data=gss_clean, method="class")
...
```{r}
set.seed(1)
gssfit <- rpart(confed ~ incumbency + pres + age + income + educ + race, data=gss_train, method="class")
